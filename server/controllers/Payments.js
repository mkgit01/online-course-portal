...
// Function to directly enroll a user in courses
exports.enrollUserInCourses = async (req, res) => {
    ...
    try {
      ...
      for (const courseId of courses) {
        if (user.courses.includes(courseId)) {
          return res.status(400).json({
            success: false,
            message: `User is already enrolled in course ${courseId}`,
          });
        }
      }
      ...
      for (const courseId of courses) {
        // Update the course to add the user to studentsEnrolled
        ...
        // Update the user to add the course to their courses array
        ...
        // Add the course progress to the user
        await User.findByIdAndUpdate(
          userId,
